...
- ⚠️ **今日 A 股因中秋节休市**：`sh000300` time=2026-09-24 收盘、YTD -4.12%，与晨间档相同。`data_minute` 返回的市场状态行 `SH_close_中秋节休市` 可确认——**遇到指数 time 落后一天时，先查市场状态行判断是否为休市，别误判为数据陈旧**。
- 结果：8 只持仓（含 SMMT Call，无变动）| 投入 228.21万 | 当前 219.93万 | 持仓收益 **-8.29万 (-3.63%)** | 总收益 **-39.77万 (-15.40%)** | 基数 258.3万（自算 total_pnl -397740.32）。
- 行情（09-25 港股收盘 / 09-24 美股收盘）：康方 93.55 (-0.37%)、海螺 15.69 (-1.38%)、亚盛 28.90 (-0.69%)、汇贤 0.325 (+1.56%)；SMMT 16.38 (-0.43%)、传奇 19.06 (+3.31%)、新氧 2.70 (-4.93%)。较同日晨间档持仓 -3.08%→-3.63%。
...
- git commit 07d5987 推送成功（3e3d287..07d5987，一次成功，5 files changed）。本日上午已有 06:01 档提交 3e3d287，本次 16:10 档覆盖更新同一份 2026-09-25.json。

## 2026-09-24 16:31 执行记录
- 方案A 全通（tencentdocs.py tdoc_call get_content）。汇率 US>RMB 6.6958 / HK>RMB 0.8531（与晨间档一致）。
- `data_quote` **本场会话全程限频**（批量 2 次全失败）→ 果断直接上 `data_minute`：7 只个股 + 2 只指数共 9 次单码调用一次跑通，未降级。**再次确认：限频就别试 data_quote，直接 data_minute。**
  - 指数 YTD 下标：`sh000300` 在 `[61]`（-4.12，与 09-23 的 -2.43 × 当日 -1.73% 吻合）；`usIXIC` 在 `[54]`（15.89，与 17.22 × 当日 -1.13% 吻合）。两指数均通过「上一档 YTD × (1+当日涨跌幅)」反推验算。下标仍会漂移，**只认能通过验算的候选值**。
- 结果：8 只持仓（含 SMMT Call，无变动）| 投入 228.21万 | 当前 221.30万 | 持仓收益 **-6.91万 (-3.03%)** | 总收益 **-38.40万 (-14.87%)** | 基数 258.3万（自算 total_pnl -384000.14）。
- 行情（09-24 港股收盘 / 09-23 美股收盘）：康方 93.90 (-3.69%)、海螺 15.91 (+0.38%)、亚盛 29.10 (-5.89%)、汇贤 0.320 (0.00%)；SMMT 16.45 (-3.91%)、传奇 18.45 (-1.02%)、新氧 2.84 (+1.43%)。全线偏弱，较同日晨间档持仓收益 -0.60%→-3.03%（-2.43pct），总收益率 -12.72%→-14.87%。
- 基准：沪深300 YTD -4.12%（CAGR 约 -1.9%）/ 纳斯达克 YTD +15.89%（CAGR 约 +12.4%）。
- 部署：workbuddy_sites_deploy 报"预留域名 portfolio-snapshot-54543 未绑定"（同每日复现，不重试）；curl 验证稳定域名 https://167b54fec43844e3986f9ea901a55bff.bj9.agentos-app.net 已含 09-24 16:31:38 数据（HTTP 200 / total_pnl -384000.14 / total_roi -14.87 / holdings_count 8），部署实际生效。
- git commit faeb62e 推送成功（43d1d67..faeb62e，一次成功，5 files changed）。
- 注：本日上午已有 06:01 档快照提交 43d1d67，本次 16:10 档覆盖更新同一份 2026-09-24.json。

## 2026-09-23 16:32 执行记录
- 方案A 全通（tencentdocs.py tdoc_call get_content）。汇率 **US>RMB 6.6958 / HK>RMB 0.8531**（较昨 6.7112/0.8552 下调，投入/当前人民币口径随之略降）。
- `data_quote` **本次会话全程限频**（批量 3 次 + 指数 1 次全失败）。果断改走 `data_minute`：7 只个股 + 2 只指数共 9 次调用一次跑通，未降级失败。**再次确认：限频就直接上 data_minute，别重试 data_quote。**
  - ⚠️ **指数 YTD 下标会漂移**：`sh000300` 本次 YTD 在 `[62]`（-2.43），而 09-22 在 `[61]`；`usIXIC` 仍在 `[54]`（17.22）。**绝不要死记下标**——用「上一档 YTD × (1+当日涨跌幅)」反推校验（本次两指数均吻合，-1.84×0.994→-2.43；16.69×1.0045→17.22）。
- 结果：8 只持仓（无变动）| 投入 228.21万 | 当前 227.47万 | 持仓收益 **-0.75万 (-0.33%)** | 总收益 **-32.23万 (-12.48%)** | 基数 258.3万（自算 total_pnl -322336.16）。
- 行情（09-23 港股收盘 / 09-22 美股收盘）：康方 97.50 (+1.67%)、海螺 15.85 (0.00%)、亚盛 30.92 (-1.59%)、汇贤 0.320 (-3.03%)；SMMT 17.12 (+1.30%)、传奇 18.64 (+0.59%)、新氧 2.80 (+2.19%)。较 09-22 档：持仓收益 -0.87%→-0.33%（+0.54pct），总收益率 -12.74%→-12.48%（+0.26pct）。
- 基准：沪深300 YTD -2.43%（CAGR -1.88%，cum_nav 0.9144）/ 纳斯达克 YTD +17.22%（CAGR +12.66%，cum_nav 1.7575）。
- 部署：workbuddy_sites_deploy 报"预留域名 portfolio-snapshot-24056 未绑定"（同每日复现，不重试）；curl 验证稳定域名 https://167b54fec43844e3986f9ea901a55bff.bj9.agentos-app.net 已含 09-23 16:32:21 数据（HTTP 200 / total_pnl -322336.16 / total_roi -12.48 / holdings_count 8），部署实际生效。
- git commit 8f6a280 推送成功（9bff75d..8f6a280，一次成功）。
- 注：本日上午已有 06:01 档快照提交 9bff75d，本次 16:10 档覆盖更新同一份 2026-09-23.json。
