...
指数（`sh000300` / `usIXIC`）同理，但 **YTD 下标因指数类型、且会随返回体字段漂移，绝不要死记**：

| 指数 | 观测到的 YTD 下标 | 实测值 |
|------|:---:|------|
| `sh000300` | 09-22 在 `[61]`；**09-23 在 `[62]`** | -2.43（09-23，与 09-22 的 -1.84 × 当日 -0.60% 吻合）|
| `usIXIC` | 稳定在 `[54]` | 17.22（09-22 收盘，与 09-21 的 16.69 × 当日 +0.45% 吻合）；其 `[61]`/`[62]` 是别的指标 |

拿到值后**必须做一次合理性交叉验算**（用上一档 YTD × (1+当日涨跌幅) 反推），否则极易取错位置。⚠️ `sh000300` 的下标已两次漂移（`[61]`→`[62]`），**只认能通过验算的那个候选值**。
...
| `data_quote` 可能**整场会话全程限频**（2026-09-21 / 09-22 连续两日实测） | 批量 + 单码 + 指数全部失败，数秒间隔的短重试**一次都没成功** | 判断依据：连续 2~3 次（间隔数秒）全失败即可认为本次会话限频。**直接对全部 7 只个股 + 2 只指数逐个走 `data_minute`**（单次调用即含快照，无批量限制），一次跑完约 9 次调用。不要反复重试 `data_quote` 浪费轮次 |
| 指数 YTD 取错下标（2026-09-22） | `usIXIC` 的 YTD 在 `[54]`，不在 `[61]`；`[61]`=6.95 是别的指标 | 用「上一档 YTD × (1+当日涨跌幅)」反推校验；两个指数的下标见步骤 4a 表格 |
| 指数 YTD 下标**漂移**（2026-09-23） | `sh000300` 的 YTD 从 `[61]` 移到 `[62]`，返回体字段数会变 | 不要把下标当固定常量；每次都用「上一档 YTD × (1+当日涨跌幅)」反推，取能通过验算的候选值 |
| 指数 quote 的 `time` 落后一个交易日，疑似"数据陈旧"（2026-09-25） | A 股遇法定节假日休市（如中秋节），指数当日无行情，`data_quote`/`data_minute` 返回的是上一交易日收盘 | **先看 `data_minute` 返回的 `qt.market` 状态行**（如 `SH_close_中秋节休市`）确认是否为休市；是休市则沿用上一交易日 YTD，属正常，不要重试或误判为陈旧数据 |
