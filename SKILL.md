...
指数（`sh000300` / `usIXIC`）同理，但 **YTD 下标因指数类型而不同，不要死记 [61]**：

| 指数 | YTD 所在下标 | 2026-09-22 实测 |
|------|:---:|------|
| `sh000300` | `[61]` | -1.84（与 09-21 的 -1.95 + 当日 +0.11 吻合）|
| `usIXIC` | `[54]` | 16.69（与 09-18 的 14.11 × (1+2.26%) 吻合）；其 `[61]`=6.95 **不是 YTD** |

拿到值后**必须做一次合理性交叉验算**（用上一档 YTD × (1+当日涨跌幅) 反推），否则极易取错位置。
